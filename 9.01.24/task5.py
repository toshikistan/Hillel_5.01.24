...
# ■ Порахуйте скільки разів цифри зустрічаються у тексті; ############### Добавил в текст цифры, чтобы было что считать.
count_digit = 0
for digit in text:
    if digit.isdigit():
        count_digit += 1
...
# ■ Порахуйте скільки разів розділові знаки зустрічаються в тексті; ############## Я попробовал сделать через symbol == " " исключить проблелы, тогда вышло 100 символов. Когда через isspace, то 88. Не совсем понял, почему.
count_symbol = 0
for symbol in text:
